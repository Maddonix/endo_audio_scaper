...
cuda_device = 0
trained_model = AudioLitModel.load_from_checkpoint(
    checkpoint_path=config["ckpt_path"]).to(cuda_device)
...
arrays = [np.zeros((channels, target_frames))
          for _ in range(batchsize_predict)]
...
st.set_page_config(page_title="Live Classification",
        ...
        while True:
            ...
            if new_information is True:
                ...
                for i, _ in enumerate(pred[0]):
                    placeholders_classes[i].text(
                        f"{classes[i]}: {round(_*100, 2)} %")
                new_information = False
