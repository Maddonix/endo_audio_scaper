...
class AudioLitModel(LightningModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.train_precision = Precision(num_classes = self.num_classes, average = None, multilabel = True)
        self.val_precision = Precision(num_classes = self.num_classes, average = None, multilabel = True)
        self.test_precision = Precision(num_classes = self.num_classes, average = None, multilabel = True)

        self.train_recall = Recall(num_classes = self.num_classes, average = None, multilabel = True)
        self.val_recall = Recall(num_classes = self.num_classes, average = None, multilabel = True)
        self.test_recall = Recall(num_classes = self.num_classes, average = None, multilabel = True)
    ...
    def training_epoch_end(self, outputs: List[Any]):
        ...
        self.metric_hist["train/prec"].append(self.trainer.callback_metrics["train/prec"])
        ...
        self.log("train/f1_best", self.metric_to_dict(max(self.metric_hist["train/acc"])), prog_bar=False)
        self.log("train/prec_best", self.metric_to_dict(max(self.metric_hist["train/loss"])), prog_bar=False)
        self.log("train/rec_best", self.metric_to_dict(max(self.metric_hist["train/loss"])), prog_bar=False)
    ...
    def validation_epoch_end(self, outputs: List[Any]):
        ...
        self.log("val/f1_best", self.metric_to_dict(max(self.metric_hist["val/acc"])), prog_bar=False)
        self.log("val/prec_best", self.metric_to_dict(max(self.metric_hist["val/loss"])), prog_bar=False)
        self.log("val/rec_best", self.metric_to_dict(max(self.metric_hist["val/loss"])), prog_bar=False)
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.1, patience=10, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08, verbose=False)

        return {"optimizer": optimizer, "lr_scheduler": lr_scheduler, "monitor": "val/loss_best"}
